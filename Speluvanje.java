...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for (int i=0; i<N; i++){
            ...
            Zbor word = new Zbor(line);

            tabela.insert(word,line);
        }
        ...
        for (int i=0; i< text.length; i++){
            ...
            if (Character.isLetter(word.charAt(word.length()-1))){
                zbor = new Zbor(word.toLowerCase());
                count = tabela.search(zbor);
                ...
                zbor = new Zbor(word.substring(0, word.length() - 1).toLowerCase());
                count = tabela.search(zbor);

                if (count == -1) {
                    res++;
                    error += word.substring(0,word.length()-1).toLowerCase() + "\n";
                }
            }
        }
        if (res == 0)
